...
class TestGradeDocument:
    ...
    def test_grade_document_relevant(self):
        ...
        grade = GradeDocument(
            is_relevant="yes",
            reasoning="Document contains information about the query topic"
        )
...
class TestGraderChain:
    ...
    def test_grade_document_relevant(self, mock_get_chain):
        ...
        mock_chain.invoke.return_value = GradeDocument(
            is_relevant="yes",
            reasoning="Document discusses AI in healthcare"
        )
    ...
    def test_grade_document_irrelevant(self, mock_get_chain):
        ...
        mock_chain.invoke.return_value = GradeDocument(
            is_relevant="no",
            reasoning="Document is about finance, not healthcare"
        )
        mock_get_chain.return_value = mock_chain

        doc = Document(
            page_content="Q3 revenue increased by 25%",
            metadata={"source": "finance.pdf"}
        )
...
class TestGradeDocuments:
    ...
    def test_grade_documents_all_irrelevant(self, mock_grade):
        ...
        mock_grade.return_value = GradeDocument(
            is_relevant="no",
