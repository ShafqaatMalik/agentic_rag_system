...
class TestFullFlow:
    ...
    async def test_query_with_rewrite(
        ...
        mock_generate.return_value = GenerationResult(
            answer="Found the answer after rewrite!",
            sources=["doc.pdf"],
            has_answer=True
        )
...
class TestNoRelevantDocsFlow:
    ...
    async def test_fallback_after_max_iterations(
        ...
        mock_rewrite.return_value = RewrittenQuery(
            rewritten_query="Still not working",
            strategy="Tried everything"
        )
        ...
        assert result["generation"] is not None
        assert "couldn't find" in result["generation"].lower() or "apologize" in result["generation"].lower()
