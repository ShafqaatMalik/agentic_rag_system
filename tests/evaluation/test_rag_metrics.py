...
class TestAnswerFaithfulness:
    ...
    def test_faithful_answer(self):
        ...
        faithfulness_score = sum(1 for fact in key_facts if fact.lower() in context.lower()) / len(
            key_facts
        )
    ...
    def test_unfaithful_answer_detection(self):
        ...
        hallucination_count = sum(1 for claim in hallucinated_claims if claim in answer)
    ...
    def test_faithfulness_with_hallucination_checker(self, mock_chain):
        ...
        from app.chains.hallucination_checker import HallucinationCheck, check_hallucination

        mock_chain_instance = MagicMock()
        mock_chain_instance.invoke = MagicMock(
            return_value=HallucinationCheck(is_grounded="yes", confidence="high", issues="None")
        )
...
class TestAnswerRelevance:
    ...
    def test_relevant_answer(self):
        ...
        relevance_score = sum(1 for kw in query_keywords if kw.lower() in answer.lower()) / len(
            query_keywords
        )
    ...
    def test_relevance_with_checker(self, mock_chain):
        ...
        from app.chains.hallucination_checker import AnswerRelevanceCheck, check_answer_relevance

        mock_chain_instance = MagicMock()
        mock_chain_instance.invoke = MagicMock(
            return_value=AnswerRelevanceCheck(
                is_relevant="yes", reasoning="Answer directly addresses the revenue question"
            )
        )
        mock_chain.return_value = mock_chain_instance
...
class TestContextRecall:
    ...
    def test_recall_calculation(self):
        ...
        # Calculate recall
        retrieved_relevant = sum(1 for doc in all_relevant_docs if doc in retrieved_content)
...
class TestRagasIntegration:
    ...
    def test_batch_evaluation_structure(self):
        ...
        batch_data = {"question": [], "answer": [], "contexts": [], "ground_truth": []}
